// Command ShodanEntitydb-mcp is an MCP server exposing the ShodanEntitydb
// SDK as MCP tools.
//
// Two tools are registered:
//
//	ShodanEntitydb_list  — list records of an entity
//	ShodanEntitydb_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: SHODAN_ENTITYDB_APIKEY carries the API key and
	// SHODAN_ENTITYDB_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SHODAN_ENTITYDB_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SHODAN_ENTITYDB_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewShodanEntitydbSDK(opts)
