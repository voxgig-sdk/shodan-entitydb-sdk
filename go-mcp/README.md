...
[MCP](https://modelcontextprotocol.io) server exposing the ShodanEntitydb SDK as
two agent tools — `shodan-entitydb_list` and `shodan-entitydb_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/shodan-entitydb-mcp)
make build
...
claude mcp add --scope user shodan-entitydb \
  -- /absolute/path/to/shodan-entitydb-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./shodan-entitydb-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/shodan-entitydb-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user shodan-entitydb \
     -- "$PWD"/dist/*/shodan-entitydb-mcp -transport stdio
   ```
...
### Call the `shodan-entitydb_list` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `shodan-entitydb_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `shodan-entitydb_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SHODAN_ENTITYDB_APIKEY` | API key sent with every request. |
| `SHODAN_ENTITYDB_BASE` | Optional override of the API base URL. |
...
```sh
./shodan-entitydb-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"shodan-entitydb_load","arguments":{"entity":"entity","query":{"id":1}}}}'
```
