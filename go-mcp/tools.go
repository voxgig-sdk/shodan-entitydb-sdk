...
type Args struct {
	Entity string         `json:"entity" jsonschema:"entity | entity_full_info | health_check | last_update"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.ShodanEntitydbSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "shodan-entitydb_list",
		Description: "List records from ShodanEntitydb. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "shodan-entitydb_load",
		Description: "Load a single record from ShodanEntitydb. " +
...
func runOp(client *sdk.ShodanEntitydbSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.ShodanEntitydbSDK, name string) (sdk.ShodanEntitydbEntity, error) {
	switch strings.ToLower(name) {
	case "entity":
		return client.Entity(nil), nil
	case "entity_full_info":
		return client.EntityFullInfo(nil), nil
	case "health_check":
		return client.HealthCheck(nil), nil
	case "last_update":
		return client.LastUpdate(nil), nil
