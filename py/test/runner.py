# ShodanEntitydb SDK test runner
...
class ShodanEntitydbTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ShodanEntitydbTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ShodanEntitydbTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ShodanEntitydbTestRunner.getenv("SHODAN_ENTITYDB_TEST_LIVE")
        override = ShodanEntitydbTestRunner.getenv("SHODAN_ENTITYDB_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ShodanEntitydbTestRunner.getenv(key)
        ...
        explain = ShodanEntitydbTestRunner.getenv("SHODAN_ENTITYDB_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ShodanEntitydbTestRunner._test_control is not None:
            return ShodanEntitydbTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ShodanEntitydbTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ShodanEntitydbTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ShodanEntitydbTestRunner.load_test_control()
...
def load_env_local():
    ShodanEntitydbTestRunner.load_env_local()


def env_override(m):
    return ShodanEntitydbTestRunner.env_override(m)


def entity_data(v):
    return ShodanEntitydbTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ShodanEntitydbTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ShodanEntitydbTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ShodanEntitydbTestRunner.load_test_control()


def live_client_options():
    return ShodanEntitydbTestRunner.live_client_options()


def live_delay_ms():
    return ShodanEntitydbTestRunner.live_delay_ms()
