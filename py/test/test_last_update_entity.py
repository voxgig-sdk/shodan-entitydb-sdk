...
class TestLastUpdateEntity:

    def test_should_create_instance(self):
        testsdk = ShodanEntitydbSDK.test(None, None)
        ent = testsdk.LastUpdate(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _last_update_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "last_update." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SHODANENTITYDB_TEST_LAST_UPDATE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        last_update_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.last_update")))
        last_update_ref01_data = None
        if len(last_update_ref01_data_raw) > 0:
            last_update_ref01_data = helpers.to_map(last_update_ref01_data_raw[0][1])

        # LOAD
        last_update_ref01_ent = client.LastUpdate(None)
        last_update_ref01_match_dt0 = {}
        last_update_ref01_data_dt0_loaded, err = last_update_ref01_ent.load(last_update_ref01_match_dt0, None)
        assert err is None
        assert last_update_ref01_data_dt0_loaded is not None



def _last_update_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/last_update/LastUpdateTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("SHODANENTITYDB_TEST_LAST_UPDATE_ENTID"))
