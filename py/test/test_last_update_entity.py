    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SHODAN_ENTITYDB_TEST_LAST_UPDATE_ENTID JSON to run live")
        client = setup["client"]
        ...
        last_update_ref01_data_dt0_loaded = last_update_ref01_ent.load(last_update_ref01_match_dt0, None)
...
def _last_update_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("SHODAN_ENTITYDB_TEST_LAST_UPDATE_ENTID"))
