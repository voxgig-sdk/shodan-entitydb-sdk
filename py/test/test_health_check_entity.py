...
class TestHealthCheckEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.HealthCheck(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _health_check_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "health_check." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SHODANENTITYDB_TEST_HEALTH_CHECK_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        health_check_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.health_check")))
        health_check_ref01_data = None
        if len(health_check_ref01_data_raw) > 0:
            health_check_ref01_data = helpers.to_map(health_check_ref01_data_raw[0][1])

        # LOAD
        health_check_ref01_ent = client.HealthCheck(None)
        health_check_ref01_match_dt0 = {}
        health_check_ref01_data_dt0_loaded, err = health_check_ref01_ent.load(health_check_ref01_match_dt0, None)
        assert err is None
        assert health_check_ref01_data_dt0_loaded is not None



def _health_check_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/health_check/HealthCheckTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("SHODANENTITYDB_TEST_HEALTH_CHECK_ENTID"))
