    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SHODANENTITYDB_TEST_ENTITY_FULL_INFO_ENTID JSON to run live")
        client = setup["client"]
        ...
        entity_full_info_ref01_data_dt0_loaded, err = entity_full_info_ref01_ent.load(entity_full_info_ref01_match_dt0, None)
        assert err is None
...
def _entity_full_info_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("SHODANENTITYDB_TEST_ENTITY_FULL_INFO_ENTID"))
