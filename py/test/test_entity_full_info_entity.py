...
class TestEntityFullInfoEntity:

    def test_should_create_instance(self):
        testsdk = ShodanEntitydbSDK.test(None, None)
        ent = testsdk.EntityFullInfo(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _entity_full_info_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "entity_full_info." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SHODAN_ENTITYDB_TEST_ENTITY_FULL_INFO_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        entity_full_info_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.entity_full_info")))
        entity_full_info_ref01_data = None
        if len(entity_full_info_ref01_data_raw) > 0:
            entity_full_info_ref01_data = helpers.to_map(entity_full_info_ref01_data_raw[0][1])

        # LOAD
        entity_full_info_ref01_ent = client.EntityFullInfo(None)
        entity_full_info_ref01_match_dt0 = {}
        entity_full_info_ref01_data_dt0_loaded = entity_full_info_ref01_ent.load(entity_full_info_ref01_match_dt0, None)
        assert entity_full_info_ref01_data_dt0_loaded is not None



def _entity_full_info_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/entity_full_info/EntityFullInfoTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "SHODAN_ENTITYDB_TEST_ENTITY_FULL_INFO_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("SHODAN_ENTITYDB_TEST_ENTITY_FULL_INFO_ENTID"))
