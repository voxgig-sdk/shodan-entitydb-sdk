...
from shodanentitydb_sdk.shodanentitydb_types import (
    EntityFullInfo,
...
class EntityFullInfoEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return EntityFullInfoEntity(self._client, opts)
    ...
    def data_get(self) -> EntityFullInfo:
    ...
    def match_get(self) -> EntityFullInfo:
    ...
    def load(self, reqmatch=None, ctrl=None) -> EntityFullInfo:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.EntityFullInfo().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
