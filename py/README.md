...
The Python SDK for the ShodanEntitydb API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Entity(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.ShodanEntitydb(None).load(
    {"id": "test01"}, None
)
