...
boru-driven command-line client **and** interactive REPL for the ShodanEntitydb
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/shodan-entitydb-cli)
make build

# 2. See usage (words, entities, env vars)
./shodan-entitydb-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/shodan-entitydb-cli
   ```
...
```sh
./shodan-entitydb-cli load 1 entity          # scalar shorthand for {id:1}
./shodan-entitydb-cli load '{id:1}' entity     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `shodan-entitydb>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/shodan-entitydb-cli`. |
...
(`./shodan-entitydb-cli <expr>`) and interactively (the REPL), and expressions compose the
