# ShodanEntitydb Ruby SDK

The Ruby SDK for the ShodanEntitydb API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Entity(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.ShodanEntitydb(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── ShodanEntitydb_sdk.rb       -- Main SDK module
