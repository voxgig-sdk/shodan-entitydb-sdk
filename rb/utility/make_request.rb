...
module ShodanEntitydbUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ShodanEntitydbResponse.new({})
    ...
    if fetch_err
      ...
      response = ShodanEntitydbResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ShodanEntitydbResponse.new(fetched)
    else
