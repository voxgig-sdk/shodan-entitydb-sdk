...
class ShodanEntitydbSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ShodanEntitydbHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ShodanEntitydbHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ShodanEntitydbFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ShodanEntitydbUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = ShodanEntitydbHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ShodanEntitydbHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ShodanEntitydbHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ShodanEntitydbError.new(
        "graphql_error", "ShodanEntitydbSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ShodanEntitydbSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
