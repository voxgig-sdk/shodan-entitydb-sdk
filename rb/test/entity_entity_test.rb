# Entity entity test

require "minitest/autorun"
require "json"
require_relative "../ShodanEntitydb_sdk"
require_relative "runner"

class EntityEntityTest < Minitest::Test
  def test_create_instance
    testsdk = ShodanEntitydbSDK.test(nil, nil)
    ent = testsdk.Entity(nil)
    assert !ent.nil?
  end

  # Feature #4: the entity stream(action, ...) method runs the op pipeline and
  # returns an Enumerator over result items. With the streaming feature active
  # it yields the feature's incremental output; otherwise it falls back to the
  # materialised list so stream always yields.
  def test_stream
    seed = {
      "entity" => {
        "entity" => {
          "s1" => { "id" => "s1" },
          "s2" => { "id" => "s2" },
          "s3" => { "id" => "s3" },
        },
      },
    }

    # Fallback: streaming inactive -> yields the materialised list items.
    base = ShodanEntitydbSDK.test(seed, nil)
    seen = base.Entity(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = ShodanEntitydbConfig.make_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = ShodanEntitydbSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
      sdk.Entity(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
        end
      end
      assert_equal 3, got.length
    end
  end

  def test_basic_flow
    setup = entity_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "entity." + _op, _live ? "live" : "unit")
      if _should_skip
        skip(_reason || "skipped via sdk-test-control.json")
        return
      end
    end
    # The basic flow consumes synthetic IDs from the fixture. In live mode
    # without an *_ENTID env override, those IDs hit the live API and 4xx.
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SHODAN_ENTITYDB_TEST_ENTITY_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # Bootstrap entity data from existing test data.
    entity_ref01_data_raw = Vs.items(Helpers.to_map(
      Vs.getpath(setup[:data], "existing.entity")))
    entity_ref01_data = nil
    if entity_ref01_data_raw.length > 0
      entity_ref01_data = Helpers.to_map(entity_ref01_data_raw[0][1])
    end

    # LIST
    entity_ref01_ent = client.Entity(nil)
    entity_ref01_match = {}

    entity_ref01_list_result = entity_ref01_ent.list(entity_ref01_match, nil)
    assert entity_ref01_list_result.is_a?(Array)

    # LOAD
    entity_ref01_match_dt0 = {
      "id" => entity_ref01_data["id"],
    }
    entity_ref01_data_dt0_loaded = entity_ref01_ent.load(entity_ref01_match_dt0, nil)
    entity_ref01_data_dt0_load_result = Helpers.to_map(entity_ref01_data_dt0_loaded.respond_to?(:data_get) ? entity_ref01_data_dt0_loaded.data_get : entity_ref01_data_dt0_loaded)
    assert !entity_ref01_data_dt0_load_result.nil?
    assert_equal entity_ref01_data_dt0_load_result["id"], entity_ref01_data["id"]

  end
end

def entity_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "entity", "EntityTestData.json")
  entity_data_source = File.read(entity_data_file)
  entity_data = JSON.parse(entity_data_source)

  options = {}
  options["entity"] = entity_data["existing"]

  client = ShodanEntitydbSDK.test(options, extra)

  # Generate idmap via transform.
  idmap = Vs.transform(
    ["entity01", "entity02", "entity03"],
    {
      "`$PACK`" => ["", {
        "`$KEY`" => "`$COPY`",
        "`$VAL`" => ["`$FORMAT`", "upper", "`$COPY`"],
      }],
    }
  )

  # Detect ENTID env override before envOverride consumes it. When live
  # mode is on without a real override, the basic test runs against synthetic
  # IDs from the fixture and 4xx's. Surface this so the test can skip.
  entid_env_raw = ENV["SHODAN_ENTITYDB_TEST_ENTITY_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "SHODAN_ENTITYDB_TEST_ENTITY_ENTID" => idmap,
    "SHODAN_ENTITYDB_TEST_LIVE" => "FALSE",
    "SHODAN_ENTITYDB_TEST_EXPLAIN" => "FALSE",
  })

  idmap_resolved = Helpers.to_map(
    env["SHODAN_ENTITYDB_TEST_ENTITY_ENTID"])
  if idmap_resolved.nil?
    idmap_resolved = Helpers.to_map(idmap)
  end

  if env["SHODAN_ENTITYDB_TEST_LIVE"] == "TRUE"
    merged_opts = Vs.merge([
      {
      },
      extra || {},
    ])
    client = ShodanEntitydbSDK.new(Helpers.to_map(merged_opts))
  end

  live = env["SHODAN_ENTITYDB_TEST_LIVE"] == "TRUE"
  {
    client: client,
    data: entity_data,
    idmap: idmap_resolved,
    env: env,
    explain: env["SHODAN_ENTITYDB_TEST_EXPLAIN"] == "TRUE",
    live: live,
    synthetic_only: live && !idmap_overridden,
    now: (Time.now.to_f * 1000).to_i,
  }
end
