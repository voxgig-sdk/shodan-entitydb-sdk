...
class EntityFullInfoEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = entity_full_info_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "entity_full_info." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SHODAN_ENTITYDB_TEST_ENTITY_FULL_INFO_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    entity_full_info_ref01_data = nil
    if entity_full_info_ref01_data_raw.length > 0
...
def entity_full_info_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "entity_full_info", "EntityFullInfoTestData.json")
  ...
  entid_env_raw = ENV["SHODAN_ENTITYDB_TEST_ENTITY_FULL_INFO_ENTID"]
