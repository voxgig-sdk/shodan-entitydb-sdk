...
class LastUpdateEntity
  ...
  def make
    opts = @_entopts.dup
    LastUpdateEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ShodanEntitydbHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [LastUpdate, Hash] the current LastUpdate data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ShodanEntitydbHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of LastUpdate fields)
  def match_get
  ...
  #
  # @param reqmatch [LastUpdateLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.LastUpdate.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [LastUpdate, Hash] the loaded LastUpdate; raises ShodanEntitydbError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ShodanEntitydbHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
