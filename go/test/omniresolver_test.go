...
import (
	...
	omni "github.com/voxgig-sdk/shodan-entitydb-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ShodanEntitydbError); is {
				out := map[string]any{
					"name":    "ShodanEntitydbError",
...
func omniCtx(arg any, client *sdk.ShodanEntitydbSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ShodanEntitydbSDK); is {
			client = live
