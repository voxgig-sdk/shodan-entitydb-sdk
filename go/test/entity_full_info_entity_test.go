...
func TestEntityFullInfoEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		entityFullInfoRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.entity_full_info")))
...
func entity_full_infoBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SHODAN_ENTITYDB_TEST_ENTITY_FULL_INFO_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SHODAN_ENTITYDB_TEST_ENTITY_FULL_INFO_ENTID"])
