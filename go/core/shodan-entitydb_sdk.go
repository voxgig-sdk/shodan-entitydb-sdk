...
func NewShodanEntitydbSDK(options map[string]any) *ShodanEntitydbSDK {
	sdk := &ShodanEntitydbSDK{
		Mode:     "live",
		Features: []Feature{},
	}
...
func (sdk *ShodanEntitydbSDK) OptionsMap() map[string]any {
...
func (sdk *ShodanEntitydbSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *ShodanEntitydbSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *ShodanEntitydbSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *ShodanEntitydbSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *ShodanEntitydbSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *ShodanEntitydbSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("ShodanEntitydbSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *ShodanEntitydbSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *ShodanEntitydbSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("ShodanEntitydbSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *ShodanEntitydbSDK {
	if sdkopts == nil {
	...
	sdk := NewShodanEntitydbSDK(sdkopts)
	sdk.Mode = "test"
