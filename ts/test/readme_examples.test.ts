...
//     (\`new ShodanEntitydbSDK(...)\` / \`ShodanEntitydbSDK.test(...)\` become
//     \`ShodanEntitydbSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ShodanEntitydbSDK'
...
const TEST_SEED = {"entity":{"entity":{"test01":{"id":"test01"}},"entity_full_info":{"test01":{"id":"test01"}},"health_check":{"test01":{"id":"test01"}},"last_update":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ShodanEntitydbSDK(...)\` and any
// \`ShodanEntitydbSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ShodanEntitydbSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ShodanEntitydbSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
