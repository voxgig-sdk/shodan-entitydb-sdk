...
describe('EntityFullInfoDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new ShodanEntitydbSDK({
  ...
  test('direct-load-entity_full_info', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-entity_full_info', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SHODAN_ENTITYDB_TEST_ENTITY_FULL_INFO_ENTID']
