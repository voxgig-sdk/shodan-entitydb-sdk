...
describe('LastUpdateDirect', async () => {
  ...
  test('direct-load-last_update', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-last_update', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SHODAN_ENTITYDB_TEST_LAST_UPDATE_ENTID']
