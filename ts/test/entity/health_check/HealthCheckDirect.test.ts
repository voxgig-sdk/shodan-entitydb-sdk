...
describe('HealthCheckDirect', async () => {
  ...
  test('direct-load-health_check', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-health_check', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SHODAN_ENTITYDB_TEST_HEALTH_CHECK_ENTID']
