# ShodanEntitydb TypeScript SDK



The TypeScript SDK for the ShodanEntitydb API — a type-safe, entity-oriented client with full async/await support.
...
| Method | Returns | Description |
| --- | --- | --- |
| `ShodanEntitydbSDK.test(testopts?, sdkopts?)` | `ShodanEntitydbSDK` | Create a test-mode client. |
...
```ts
const health_check = await client.HealthCheck().load()
```
...
shodan-entitydb/
├── src/
│   ├── ShodanEntitydbSDK.ts        # Main SDK class
