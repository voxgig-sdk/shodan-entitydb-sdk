...
local ShodanEntitydbSDK = {}
ShodanEntitydbSDK.__index = ShodanEntitydbSDK
...
ShodanEntitydbSDK._make_feature = _make_feature


function ShodanEntitydbSDK.new(options)
  local self = setmetatable({}, ShodanEntitydbSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ShodanEntitydbSDK:options_map()
...
function ShodanEntitydbSDK:get_utility()
  return Utility.copy(self._utility)
end


function ShodanEntitydbSDK:get_root_ctx()
  return self._rootctx
end


function ShodanEntitydbSDK:prepare(fetchargs)
...
function ShodanEntitydbSDK:direct(fetchargs)
...
-- by passing it.
function ShodanEntitydbSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ShodanEntitydbSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ShodanEntitydbSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:EntityFullInfo():list() / client:EntityFullInfo():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
-- Idiomatic facade: client:HealthCheck():list() / client:HealthCheck():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
-- Idiomatic facade: client:LastUpdate():list() / client:LastUpdate():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
function ShodanEntitydbSDK.test(testopts, sdkopts)
  ...
  local sdk = ShodanEntitydbSDK.new(sdkopts)
  sdk.mode = "test"
