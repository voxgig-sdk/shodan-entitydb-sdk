...
local HealthCheckEntity = {}
HealthCheckEntity.__index = HealthCheckEntity


function HealthCheckEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HealthCheckEntity)
  self._name = "health_check"
  self._client = client
...
function HealthCheckEntity:get_name()
  return self._name
end


function HealthCheckEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HealthCheckEntity.new(self._client, opts)
end
...
function HealthCheckEntity:mark_deleted()
  self._deleted = true
end


function HealthCheckEntity:deleted()
  return true == self._deleted
end


function HealthCheckEntity:data_set(args)
  if args ~= nil then
...
function HealthCheckEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HealthCheckEntity:match_set(args)
  if args ~= nil then
...
function HealthCheckEntity:match_get()
...
function HealthCheckEntity:stream(action, args, callopts)
...
---@param reqmatch HealthCheckLoadMatch
---@param ctrl? table
---@return HealthCheck
---@return string? err
function HealthCheckEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function HealthCheckEntity:_run_op(ctx, post_done)
  local utility = self._utility
