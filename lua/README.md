...
The Lua SDK for the ShodanEntitydb API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Entity(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:ShodanEntitydb(nil):load(
  { id = "test01" }, nil
)
