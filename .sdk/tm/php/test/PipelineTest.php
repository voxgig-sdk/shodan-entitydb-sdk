...
// ShodanEntitydb SDK pipeline test
//
...
{
    private static function utility(): ShodanEntitydbUtility
    {
        return new ShodanEntitydbUtility();
    }
    ...
    private static function ctx(array $over = []): ShodanEntitydbContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ShodanEntitydbContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ShodanEntitydbOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ShodanEntitydbError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ShodanEntitydbOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ShodanEntitydbMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ShodanEntitydbOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ShodanEntitydbMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ShodanEntitydbOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ShodanEntitydbMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ShodanEntitydbSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ShodanEntitydbMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ShodanEntitydbResponse([]),
            'result' => new ShodanEntitydbResult([]),
        ]);
        [, $err] = ShodanEntitydbMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ShodanEntitydbSpec([]),
            'response' => null,
            'result' => new ShodanEntitydbResult([]),
        ]);
        [, $err] = ShodanEntitydbMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ShodanEntitydbSpec(['step' => 's']),
            'response' => new ShodanEntitydbResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ShodanEntitydbResult([]),
        ]);
        [, $err] = ShodanEntitydbMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ShodanEntitydbSpec(['step' => 's']),
            'response' => new ShodanEntitydbResponse(self::resp(200, ['v' => 1])),
            'result' => new ShodanEntitydbResult([]),
        ]);
        [, $err] = ShodanEntitydbMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ShodanEntitydbSpec(['step' => 's']),
            'response' => new ShodanEntitydbResponse(self::resp(200, ['v' => 2])),
            'result' => new ShodanEntitydbResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ShodanEntitydbMakeResponse::call($ctx);
    ...
    {
        $preset = new ShodanEntitydbResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ShodanEntitydbSpec([]),
            'response' => new ShodanEntitydbResponse([]),
            'result' => new ShodanEntitydbResult([]),
        ...
        [$got, $err] = ShodanEntitydbMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ShodanEntitydbResult([])]);
        [, $err] = ShodanEntitydbMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ShodanEntitydbSpec([]), 'result' => null]);
        [, $err] = ShodanEntitydbMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ShodanEntitydbSpec(['step' => 's']),
            'result' => new ShodanEntitydbResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ShodanEntitydbOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ShodanEntitydbMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ShodanEntitydbSpec(['step' => 's']),
            'result' => new ShodanEntitydbResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ShodanEntitydbOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ShodanEntitydbMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ShodanEntitydbResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ShodanEntitydbSpec([]),
            'result' => new ShodanEntitydbResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ShodanEntitydbMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ShodanEntitydbMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ShodanEntitydbError('boom', 'boom');
        $utility->fetcher = function (ShodanEntitydbContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ShodanEntitydbSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ShodanEntitydbMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ShodanEntitydbContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ShodanEntitydbMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ShodanEntitydbResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ShodanEntitydbMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ShodanEntitydbContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ShodanEntitydbSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ShodanEntitydbMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ShodanEntitydbResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ShodanEntitydbSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ShodanEntitydbMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ShodanEntitydbMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ShodanEntitydbMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ShodanEntitydbResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ShodanEntitydbDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ShodanEntitydbResult(['ok' => false])]);
        $this->expectException(ShodanEntitydbError::class);
        ShodanEntitydbDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ShodanEntitydbResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ShodanEntitydbMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ShodanEntitydbMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ShodanEntitydbBaseFeature();
        $b = new ShodanEntitydbBaseFeature();
        ShodanEntitydbFeatureAdd::call($ctx, $a);
        ShodanEntitydbFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ShodanEntitydbBaseFeature
    {
        $f = new ShodanEntitydbBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ShodanEntitydbFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ShodanEntitydbContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ShodanEntitydbSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ShodanEntitydbPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        ShodanEntitydbPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ShodanEntitydbResponse(['headers' => null]),
            'result' => new ShodanEntitydbResult([]),
        ]);
        ShodanEntitydbResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ShodanEntitydbResultBody::call($ctx);
