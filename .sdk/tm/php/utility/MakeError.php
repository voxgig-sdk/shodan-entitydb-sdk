...
class ShodanEntitydbMakeError
{
    public static function call(?ShodanEntitydbContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ShodanEntitydbContext([], null);
        }
        $op = $ctx->op ?? new ShodanEntitydbOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ShodanEntitydbResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ShodanEntitydbError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ShodanEntitydbError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof ShodanEntitydbError) {
