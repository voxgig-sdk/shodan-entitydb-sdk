...
// ShodanEntitydb SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ShodanEntitydbTestFeature extends ShodanEntitydbBaseFeature
{
    ...
    public function init(ShodanEntitydbContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ShodanEntitydbContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ShodanEntitydbParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ShodanEntitydbContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ShodanEntitydbContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ShodanEntitydbParam::call($ctx, $k);
