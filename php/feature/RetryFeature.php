...
// ShodanEntitydb SDK retry feature
...
class ShodanEntitydbRetryFeature extends ShodanEntitydbBaseFeature
{
    ...
    public function init(ShodanEntitydbContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ShodanEntitydbContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ShodanEntitydbContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ShodanEntitydbContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
