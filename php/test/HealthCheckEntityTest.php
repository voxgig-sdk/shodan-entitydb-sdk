...
class HealthCheckEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ShodanEntitydbSDK::test(null, null);
        $ent = $testsdk->HealthCheck(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = health_check_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "health_check." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SHODAN_ENTITYDB_TEST_HEALTH_CHECK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $health_check_ref01_data = null;
        if (count($health_check_ref01_data_raw) > 0) {
...
function health_check_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/health_check/HealthCheckTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["health_check01", "health_check02", "health_check03"] as $k) {
    ...
    $entid_env_raw = getenv("SHODAN_ENTITYDB_TEST_HEALTH_CHECK_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["SHODAN_ENTITYDB_TEST_HEALTH_CHECK_ENTID"]);
