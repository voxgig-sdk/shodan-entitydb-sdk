...
// ShodanEntitydb SDK feature test
//
...
class FtCtrl extends ShodanEntitydbControl
{
...
class FtClient
{
    ...
    private ShodanEntitydbUtility $utility;

    public function __construct(ShodanEntitydbUtility $utility, array $options)
    {
    ...
    public function get_utility(): ShodanEntitydbUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ShodanEntitydbContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ShodanEntitydbContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ShodanEntitydbContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ShodanEntitydbConfig::make_feature($name);
    ...
    private static function buildUrl(ShodanEntitydbSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ShodanEntitydbError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ShodanEntitydbResponse($fetched);
        }
    ...
    private function fail(ShodanEntitydbContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ShodanEntitydbContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
