...
//    only - a typed ShodanEntitydbContext there would make every ctx
...
final class ShodanEntitydbOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ShodanEntitydbContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new ShodanEntitydbSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new ShodanEntitydbResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new ShodanEntitydbError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new ShodanEntitydbResponse($clean['response']);
    ...
    public static function err_from_map($m): ?ShodanEntitydbError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ShodanEntitydbError(is_string($code) ? $code : '', $msg);
    }
